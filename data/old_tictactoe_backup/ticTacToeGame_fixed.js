...
class RateLimitQueue {
    constructor() {
        ...
        this.minInterval = 1000; // 최소 1초 간격
    }
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (error.response && error.response.status === 429 && i < maxRetries - 1) {
                ...
                console.log(`[TicTacToe] Rate limit hit, retrying after ${delay}ms...`);
...
return`<div class="${cls}">${cell==='X'?'❌':cell==='O'?'⭕':''}</div>`;
            ...
            // Rate limit 큐를 통해 API 호출
            const imageUrl = await rateLimitQueue.add(async () => {
                return await retryWithBackoff(async () => {
                    console.log('[TicTacToe] HCTI API 호출 중...');
            ...
            console.log('[TicTacToe] 이미지 생성 및 캐시 저장 완료');
            ...
            if (error.response) {
                console.error('[TicTacToe] API 응답:', {
