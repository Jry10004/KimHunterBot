...
const TIC_TAC_TOE_GAME = {
    ...
    // SVG 생성 (최적화된 버전)
    createBoardSVG(board, player1, player2, winPattern = [], lastMovePosition = null) {
        const xCount = board.filter(cell => cell === 'X').length;
        const oCount = board.filter(cell => cell === 'O').length;
        const isXActive = xCount <= oCount;
        
        // 미리 정의된 스타일로 SVG 크기 최소화
        const svg = `<svg width="450" height="550" xmlns="http://www.w3.org/2000/svg">
<defs>
<style>
.bg{fill:#2C2F33}
.title{fill:#FFF;font:bold 32px Arial}
.active{fill:#5865F2}
.inactive{fill:#99AAB5}
.txt{fill:#FFF;font:16px Arial}
.turn{fill:${isXActive ? '#FF6B6B' : '#4ECDC4'};font:18px Arial}
.rule{fill:#FFD93D;font:14px Arial}
.board{fill:#23272A}
.cell{fill:#40444B}
.win{fill:#FFD700;opacity:0.3}
.last{fill:#98FB98;opacity:0.3}
.x{fill:#FF6B6B;font:48px Arial}
.o{fill:#4ECDC4;font:48px Arial}
.count{fill:#FFF;font:16px Arial}
</style>
</defs>
<rect class="bg" width="450" height="550"/>
<text class="title" x="225" y="40" text-anchor="middle">TIC TAC TOE</text>
<rect class="${isXActive?'active':'inactive'}" x="30" y="70" width="160" height="40" rx="8"/>
<text class="txt" x="110" y="95" text-anchor="middle">❌ ${player1.username.substring(0,10)}</text>
<text class="txt" x="225" y="95" text-anchor="middle" font-weight="bold" font-size="20">VS</text>
<rect class="${!isXActive?'active':'inactive'}" x="260" y="70" width="160" height="40" rx="8"/>
<text class="txt" x="340" y="95" text-anchor="middle">⭕ ${player2.username.substring(0,10)}</text>
<text class="turn" x="225" y="140" text-anchor="middle">${isXActive ? player1.username : player2.username}님의 턴!</text>
<text class="rule" x="225" y="165" text-anchor="middle">※ 각 플레이어는 최대 3개까지만 놓을 수 있습니다</text>
<rect class="board" x="65" y="185" width="320" height="320" rx="12"/>
<line x1="165" y1="195" x2="165" y2="495" stroke="white" stroke-width="3"/>
<line x1="285" y1="195" x2="285" y2="495" stroke="white" stroke-width="3"/>
<line x1="75" y1="285" x2="375" y2="285" stroke="white" stroke-width="3"/>
<line x1="75" y1="395" x2="375" y2="395" stroke="white" stroke-width="3"/>
${board.map((cell, i) => {
    const row = Math.floor(i / 3);
    const col = i % 3;
    const x = 75 + col * 100 + 50;
    const y = 195 + row * 100 + 50;
    
    let bg = '';
    if (winPattern.includes(i)) bg = `<rect class="win" x="${x-40}" y="${y-40}" width="80" height="80" rx="8"/>`;
    else if (i === lastMovePosition) bg = `<rect class="last" x="${x-40}" y="${y-40}" width="80" height="80" rx="8"/>`;
    
    let content = '';
    if (cell === 'X') content = `<text class="x" x="${x}" y="${y+10}" text-anchor="middle">❌</text>`;
    else if (cell === 'O') content = `<text class="o" x="${x}" y="${y+10}" text-anchor="middle">⭕</text>`;
    
    return bg + content;
}).join('')}
<text class="count" x="75" y="530">❌ 남은 수: ${3 - Math.min(this.activeGames.get(player1.id + player2.id)?.moveHistory?.X?.length || 0, 3)}</text>
<text class="count" x="375" y="530" text-anchor="end">⭕ 남은 수: ${3 - Math.min(this.activeGames.get(player1.id + player2.id)?.moveHistory?.O?.length || 0, 3)}</text>
</svg>`;
    ...
    async createBoardImage(board, player1, player2, winPattern = [], lastMovePosition = null) {
        try {
            ...
            if (cached) {
                // 캐시된 버퍼로 새 attachment 생성
                const buffer = Buffer.from(cached);
                return new AttachmentBuilder(buffer, { name: 'tictactoe.svg' });
            }
            
            // SVG 생성
            const svg = this.createBoardSVG(board, player1, player2, winPattern, lastMovePosition);
            ...
            // AttachmentBuilder 생성
            const attachment = new AttachmentBuilder(svgBuffer, { name: 'tictactoe.svg' });
    ...
    // 초기화
    async initialize() {
        console.log('[TicTacToe] SVG 직접 렌더링 시스템 초기화');
    }
};
