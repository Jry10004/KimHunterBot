...
const DAILY_MISSIONS = {
    ...
    earnGold: { name: '💰 골드 2,500,000 벌기', desc: '어떤 방법이든 2,500,000골드 획득' },  // 50,000 -> 2,500,000 (50x)
...
const WEEKLY_MISSIONS = {
    ...
    totalGoldEarned: { name: '💎 골드 5000만 벌기', desc: '일주일간 총 5000만 골드 획득' },  // 100만 -> 5000만 (50x)
...
async function showDailyMissions(interaction) {
    ...
    embed.addFields({
        name: '🎁 완료 보상',
        value: '💰 5,000,000 골드\n💎 엠블럼 강화석 x1',  // 100,000 -> 5,000,000 (50x)
        inline: true
    });
...
async function showWeeklyMissions(interaction) {
    ...
    embed.addFields({
        name: '💎 완료 보상',
        value: '💰 25,000,000 골드\n💎 엠블럼 강화석 x5\n🎁 프리미엄 코인 x1',  // 500,000 -> 25,000,000 (50x)
        inline: true
    });
...
async function claimDailyMissionReward(interaction) {
    ...
    if (!missionData || missionData.dailyCompletedCount < 10 || missionData.dailyRewardClaimed) {
        if (interaction.deferred || interaction.replied) {
            return await interaction.editReply({ content: '❌ 보상을 수령할 수 없습니다.' });
        } else {
    ...
    // 보상 지급 (칭호 효과 적용)
    const goldReward = applyGoldBonus(5000000, user);  // 100000 -> 5000000 (50x)
    ...
    await MissionHelper.updateGoldEarned(interaction.user.id, 5000000);  // 100000 -> 5000000 (50x)
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '획득 보상', value: '💰 5,000,000 골드\n💎 엠블럼 강화석 x1', inline: true },  // 100,000 -> 5,000,000 (50x)
...
async function claimWeeklyMissionReward(interaction) {
    ...
    // 보상 지급 (칭호 효과 적용)
    const weeklyGoldReward = applyGoldBonus(25000000, user);  // 500000 -> 25000000 (50x)
    ...
    // 골드 획득 미션 업데이트
    const MissionHelper = require('../../utils/missionHelper');
    await MissionHelper.updateGoldEarned(interaction.user.id, 25000000);  // 500000 -> 25000000 (50x)
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '획득 보상', value: '💰 25,000,000 골드\n💎 엠블럼 강화석 x5\n🎁 프리미엄 코인 x1', inline: false }  // 500,000 -> 25,000,000 (50x)
        )
