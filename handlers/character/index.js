...
async function handleCharacterInteraction(interaction) {
    ...
    // 엠블럼 관련
    ...
    else if (customId === 'emblem_shop_stone') {
        // 강화석 구매 메뉴 표시
        ...
        const embed = new EmbedBuilder()
            .setColor('#FFD700')
            .setTitle('💎 엠블럼 강화석 상점')
            .setDescription('엠블럼을 강화하는데 필요한 강화석을 구매할 수 있습니다.')
            .addFields(
            ...
            .setFooter({ text: '구매할 아이템을 선택하세요!' });
            
        const buttons = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('emblem_shop_buy_stone')
                    .setLabel('💎 강화석 1개')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(user.gold < 50000),
                new ButtonBuilder()
                    .setCustomId('emblem_shop_buy_stone_10')
                    .setLabel('💎 강화석 10개')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(user.gold < 450000),
                new ButtonBuilder()
                    .setCustomId('emblem_shop_buy_blessing')
                    .setLabel('✨ 축복 주문서')
                    .setStyle(ButtonStyle.Success)
                    .setDisabled(user.gold < 200000),
                new ButtonBuilder()
                    .setCustomId('emblem_shop_buy_protection')
                    .setLabel('🛡️ 보호 주문서')
                    .setStyle(ButtonStyle.Success)
                    .setDisabled(user.gold < 150000)
            );
    ...
    else if (customId === 'emblem_shop_buy_stone') {
    ...
    else if (customId === 'emblem_shop_buy_stone_10') {
        const { handleEmblemShopInteraction } = require('../../systems/emblemShop');
        const user = await getUser(interaction.user.id);
        return await handleEmblemShopInteraction(interaction, getUser, saveUser);
    }
    else if (customId === 'emblem_shop_buy_blessing') {
        const { handleEmblemShopInteraction } = require('../../systems/emblemShop');
        const { getUser, saveUser } = require('../common/utils');
        return await handleEmblemShopInteraction(interaction, getUser, saveUser);
    }
    else if (customId === 'emblem_shop_buy_protection') {
        const { handleEmblemShopInteraction } = require('../../systems/emblemShop');
        const { getUser, saveUser } = require('../common/utils');
        return await handleEmblemShopInteraction(interaction, getUser, saveUser);
    }
